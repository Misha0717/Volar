            ...
            for i=1, #coffre,1 do
                if coffre[i].name == item then
                    if (coffre[i].count >= count and count > 0) then
                        local remaining = math.floor((xPlayer.maxWeight - currentWeight) / sourceItem.weight)
                        if sourceItem.weight ~= -1 and ((count) > remaining) then
        ...
        TriggerEvent('esx_trunk:getSharedDataStore',  plate, function(store)
            ...
            for i=1, #storeWeapons, 1 do
                if storeWeapons[i].name == item then
                    ...
                    component = storeWeapons[i].components
                    weaponTint = storeWeapons[i].tintIndex
            ...
            xPlayer.setWeaponTint(weaponName, weaponTint)
            for k,v in ipairs(component) do
                xPlayer.addWeaponComponent(weaponName, v)
            end
            local identifier = GetPlayerIdentifiers(_source)[1]
            --WeaponName = item
            MySQL.Async.execute("UPDATE weapons SET identifier = @identifier WHERE identifier = @identifiers AND WeaponName = @WeaponName",{
                ['@identifier'] = identifier,
                ['@identifiers'] = "trunk: "..plate,
...
AddEventHandler('esx_trunk:putItem', function(plate, type, item, count,max, owned)
    ...
    local _source      = source
    local xPlayer      = ESX.GetPlayerFromId(_source)
    local xPlayerOwner = GetPlayerIdentifiers(_source)[1]
    ...
    if type == 'item_weapon' then
        ...
        TriggerEvent('esx_trunk:getSharedDataStore', plate, function(store)
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item))) > max then
                ...
                local plss = "trunk: "..plate..""
                MySQL.Async.execute("UPDATE weapons SET identifier = @identifier WHERE identifier = @identifiers AND WeaponName = @WeaponName",{
                    ['@identifier'] = plss,
                    ['@identifiers'] = xPlayerOwner,
                    ['@WeaponName'] = item
                })
